			...
			*/
			if (file_found)
			{
				...
				while (1)
				{
					...
					{
						sprintf(response, "Please choose Y (for yes) or N (for no) ", arg);
						send(clientSocket, response, strlen(response), 0);
					}
				}
			}
